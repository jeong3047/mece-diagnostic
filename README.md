# MECE Diagnostic

**AI 코드 어시스턴트를 위한 구조화된 문제 분석 프레임워크**

McKinsey의 MECE(Mutually Exclusive, Collectively Exhaustive) 원칙과 Diagnostic Pruning 기법을 AI 코딩 어시스턴트(Claude Code)에 적용한 스킬입니다.

---

## 목차

- [왜 만들었는가](#왜-만들었는가)
- [한눈에 보는 비교](#한눈에-보는-비교)
- [MECE란?](#mece란)
- [프레임워크 흐름](#프레임워크-흐름)
  - [Issue Tree — 문제를 MECE하게 분해](#issue-tree--문제를-mece하게-분해)
  - [Why Tree — 원인을 깊이 파고들기](#why-tree--원인을-깊이-파고들기)
  - [Pruning — 검증하고 가지치기](#pruning--검증하고-가지치기)
- [Adaptive Depth — 복잡도별 분석 깊이](#adaptive-depth--복잡도별-분석-깊이)
- [실제 적용 사례: 기술 SEO 분석](#실제-적용-사례-기술-seo-분석)
- [발표용 예시 모음](#발표용-예시-모음)
- [장점](#장점)
- [사용법](#사용법)
- [파일 구조](#파일-구조)
- [배경](#배경)

---

## 왜 만들었는가

### 계기 1: AI가 첫 번째 원인에만 달려드는 문제

AI에게 원인 분석을 맡기면, **첫 번째로 떠오른 원인에 바로 달려듭니다.** 가능한 원인을 빠짐없이 나열하고 하나씩 검증하며 기각하는 과정이 빠져 있습니다.

```
❌ 기존 AI 분석 흐름

  "배포 후 페이지 느려짐"
       │
       ▼
  "번들 사이즈 문제인 것 같습니다"  ← 첫 번째 추측에 바로 달려듦
       │
       ▼
  번들 최적화 시도... 실패
       │
       ▼
  "그러면 이미지 최적화를..."  ← 또 추측
       │
       ▼
  ♻️ 반복 (진짜 원인: DB 쿼리 N+1 문제)
```

### 계기 2: 외부 팀에서 쏟아지는 확인 요청에 즉시 대응

실제로 다른 팀에서 개발 측으로 확인 요청이 빈번하게 들어옵니다.

- "이 API 응답이 느린데 프론트 문제인가요?"
- "이 데이터가 이상한데 로직 버그인가요?"
- "배포 후 이 기능이 안 되는데 확인해주세요"

이런 요청이 올 때마다 하나하나 수동으로 파악하면 시간이 걸리고, 빠뜨리는 항목이 생깁니다. MECE Diagnostic을 통해 **가능한 원인을 구조적으로 전수 파악하고, 즉시 대응**할 수 있게 하고 싶었습니다.

"확인해봤는데 프론트 문제는 아닌 것 같습니다"가 아니라, **"이 5가지 가능성을 모두 확인했고, 3번이 원인입니다"**라고 근거와 함께 답할 수 있어야 합니다.

**MECE Diagnostic은 이 두 가지 문제를 해결합니다.**

> AI를 통해 확인하는 모든 사항을 — 원인 분석, 아키텍처 결정, 성능 조사, 외부 확인 요청 대응 등 — 구조화된 프레임워크로 사전 검증하여, **빠짐없이 검증된 답**을 얻기 위한 도구입니다.

---

## 한눈에 보는 비교

| 기존 AI 분석 | MECE Diagnostic 적용 |
|---|---|
| 원인을 하나 추측하고 바로 수정 | 가능한 원인을 **전수 나열**(Issue Tree)한 뒤 검증 |
| 반증 없이 가설에 집착 | 반증되면 **즉시 기각**(Kill Fast) |
| 기각한 가설은 잊어버림 | **Pruning 기록**으로 추적 |
| "충분히 찾은 것 같다"로 마무리 | **Coverage Gate** — 모든 가지를 판정해야 결론 가능 |
| 결론이 나열식 | **피라미드 원칙** — 결론 먼저, 근거는 아래에 |

---

## MECE란?

**M**utually **E**xclusive, **C**ollectively **E**xhaustive — 상호배타적이고, 전체를 포괄하는 분해.

```
  ❌ MECE가 아닌 분해                    ✅ MECE인 분해

  "페이지가 느린 이유"                   "페이지가 느린 이유"

  ┌──────────┐ ┌──────────┐            ┌──────────┐ ┌──────────┐ ┌──────────┐
  │  렌더링   │ │  API     │            │ 네트워크  │ │  서버    │ │ 클라이언트│
  │  문제    │ │  느림    │            │ (전송)   │ │ (처리)   │ │ (렌더링) │
  └──────────┘ └──────────┘            └──────────┘ └──────────┘ └──────────┘
       ↑                                    ↑           ↑           ↑
  "API 느림"이 렌더링과                 겹침 없음(ME) + 빠짐 없음(CE)
  겹칠 수 있음 → 누락도 있음            = 응답시간의 수학적 분해
```

> MECE가 아니면 "이미 확인한 것"을 다시 확인하거나, "아무도 확인 안 한 것"이 생깁니다.

---

## 프레임워크 흐름

```mermaid
flowchart TD
    START["문제 발생 / 확인 요청 접수"] --> DEF["0. 문제 정의<br/>경계 | 성공 기준 | 시간"]
    DEF --> DAY1["0-1. Day 1 Answer<br/>'지금 아는 것만으로 답하면?'"]
    DAY1 --> DEPTH{"복잡도 판단"}

    DEPTH -->|"원인 1~2개"| LIGHT["Light"]
    DEPTH -->|"원인 복수"| STD["Standard"]
    DEPTH -->|"시스템 레벨"| DEEP["Deep"]

    LIGHT --> IT["1. Issue Tree<br/>MECE 분해"]
    STD --> IT
    DEEP --> IT

    IT --> BR["1-1. Branch Registry<br/>말단 가지 전수 등록"]
    BR --> WT["2. Why Tree<br/>원인 MECE 분해"]
    WT --> HYP["3. 가설 설정<br/>데이터 전에 가설 먼저"]
    HYP --> PM["4. Priority Matrix<br/>Impact × Evidence"]
    PM --> VERIFY["5. 검증 + Pruning<br/>반증 → 즉시 기각"]
    VERIFY --> CG{"6. Coverage Gate<br/>⬜ = 0개?"}

    CG -->|"⬜ 남음"| VERIFY
    CG -->|"모두 판정"| SW["7. So What?<br/>'그래서 뭐?'"]

    SW --> PYRAMID["8. 결론<br/>피라미드 원칙"]

    style START fill:#ff6b6b,color:#fff
    style CG fill:#f39c12,color:#fff
    style PYRAMID fill:#2ecc71,color:#fff
```

아래에서 핵심 단계를 구체적인 예시와 함께 설명합니다.

### Issue Tree — 문제를 MECE하게 분해

문제를 최상위 질문에서 시작하여 하위 이슈로 쪼갭니다.

```mermaid
graph TD
    Q["🔴 배포 후 LCP 3초 → 5초로 악화"]
    Q --> A["네트워크 계층"]
    Q --> B["서버 계층"]
    Q --> C["클라이언트 계층"]

    A --> A1["CDN 캐시 미스"]
    A --> A2["리소스 용량 증가"]

    B --> B1["API 응답 지연"]
    B --> B2["SSR 렌더링 지연"]

    C --> C1["JS 번들 사이즈"]
    C --> C2["메인스레드 블로킹"]
    C --> C3["레이아웃 시프트"]

    style Q fill:#ff6b6b,color:#fff
    style A fill:#4ecdc4,color:#fff
    style B fill:#45b7d1,color:#fff
    style C fill:#96ceb4,color:#fff
```

모든 **말단 가지**(A1~C3)를 Branch Registry에 등록합니다. **⬜가 하나라도 남으면 결론 불가** — 이것이 Coverage Gate입니다.

| # | 가지 | 판정 | 근거 |
|---|------|------|------|
| 1 | CDN 캐시 미스 | ⬜ | |
| 2 | 리소스 용량 증가 | ⬜ | |
| 3 | API 응답 지연 | ⬜ | |
| 4 | SSR 렌더링 지연 | ⬜ | |
| 5 | JS 번들 사이즈 | ⬜ | |
| 6 | 메인스레드 블로킹 | ⬜ | |
| 7 | 레이아웃 시프트 | ⬜ | |

### Why Tree — 원인을 깊이 파고들기

Issue Tree에서 이슈를 발견하면, **"왜?"**를 반복하며 근본 원인까지 내려갑니다. 이때 4가지 분해 전략 중 적절한 것을 선택합니다.

```mermaid
graph TD
    P["🔍 API 응답 지연 — 이슈 발견"]
    P --> W1["왜? DB 쿼리 느림"]
    P --> W2["왜? 외부 API 호출 추가됨"]
    P --> W3["왜? 캐시 만료"]

    W1 --> W1a["왜? N+1 쿼리 발생"]
    W1 --> W1b["왜? 인덱스 미적용"]

    W1a --> ROOT["🎯 근본 원인: 배포된 코드에서<br/>관계 데이터를 루프 내 개별 조회"]

    style P fill:#ff6b6b,color:#fff
    style ROOT fill:#2ecc71,color:#fff
    style W1 fill:#e74c3c,color:#fff
    style W2 fill:#95a5a6,color:#fff
    style W3 fill:#95a5a6,color:#fff
    style W1a fill:#e74c3c,color:#fff
    style W1b fill:#95a5a6,color:#fff
```

**분해 전략 4종** — 상황에 맞는 전략으로 MECE하게 쪼갭니다:

```
 수학적 분해                          프로세스 분해
 수식으로 나눈다                       흐름 단계별로 나눈다

 응답시간 = 네트워크 + 서버 + 렌더링    요청 → 인증 → 라우팅 → 로직 → DB → 응답
              8%      72%★   20%               OK     OK      OK    ★느림!

 → 가장 큰 비중부터 조사               → 어느 단계에서 병목인지 특정
 ─────────────────────────────────────────────────────────────────────
 세그먼트 분해                         내부-외부 분해
 범주별로 나눈다                        통제 가능 여부로 나눈다

 ┌───────┐ ┌─────────┐ ┌───────┐     ┌─── 내부 (통제 가능) ──┐
 │  iOS  │ │ Android │ │  Web  │     │ 코드 변경 / 설정 오류  │
 │ 정상  │ │  정상   │ │★ 느림 │     ├─── 외부 (통제 불가) ──┤
 └───────┘ └─────────┘ └───────┘     │ 외부 API / CDN / 인프라│
                                     └───────────────────────┘
 → Web에서만 발생 → Web 코드 조사      → 대응 전략이 달라짐
```

### Pruning — 검증하고 가지치기

가설을 세우고 반증 데이터로 검증합니다. 반증되면 미련 없이 기각(Kill Fast).

```mermaid
graph LR
    H1["가설 1: CDN 캐시 미스"] -->|"캐시 히트율 98%<br/>→ 반증됨"| R1["✅ 기각"]
    H2["가설 2: JS 번들 증가"] -->|"번들 사이즈 변화 없음<br/>→ 반증됨"| R2["✅ 기각"]
    H3["가설 3: API 응답 지연"] -->|"평균 200ms → 1800ms<br/>→ 확인됨"| R3["🔍 이슈 발견"]
    H4["가설 4: SSR 지연"] -->|"SSR 시간 변화 없음<br/>→ 반증됨"| R4["✅ 기각"]

    R3 --> DEEP["Why Tree로<br/>근본 원인 추적"]

    style R1 fill:#95a5a6,color:#fff
    style R2 fill:#95a5a6,color:#fff
    style R3 fill:#e74c3c,color:#fff
    style R4 fill:#95a5a6,color:#fff
    style DEEP fill:#2ecc71,color:#fff
```

기각된 가설은 Pruning 기록에 남겨 같은 검증을 반복하지 않습니다.

---

## 실제 적용 사례: 기술 SEO 분석

Next.js 프로젝트의 기술 SEO 전수 점검에 MECE Diagnostic을 적용한 사례입니다.

### 문제 정의

| 항목 | 내용 |
|------|------|
| **경계** | 프론트엔드 기술 SEO만. 콘텐츠 전략/백링크 제외 |
| **성공 기준** | 크롤링 효율 95%+, CWV 양호, 미색인 0건 |
| **시간 프레임** | 2주 내 긴급/높음 항목 완료 |

### Issue Tree — 기술 SEO를 MECE 분해

```mermaid
graph TD
    Q["🔴 기술 SEO 전수 점검"]
    Q --> T1["1. 크롤링 낭비"]
    Q --> T2["2. 성능 및 인덱싱"]
    Q --> T3["3. 프레임워크 구조"]
    Q --> T4["4. Core Web Vitals"]
    Q --> T5["5. 콘텐츠 시그널"]
    Q --> T6["6. GEO<br/>(AI 검색 최적화)"]

    T1 --> T1a["중복 canonical"]
    T1 --> T1b["JS 캐시 헤더 누락<br/>+ TinyMCE 전역 로드"]
    T1 --> T1c["404 비율 3.23%"]
    T1 --> T1d["NOINDEX ↔ robots.txt 불일치"]

    T4 --> T4a["히어로 배너 SSR 미포함"]
    T4 --> T4b["렌더 블로킹 리소스"]
    T4 --> T4c["Preconnect 누락"]
    T4 --> T4d["이미지 WebP 미적용"]
    T4 --> T4e["폰트 preload 누락"]
    T4 --> T4f["스켈레톤 ↔ 실제 높이 불일치"]

    style Q fill:#ff6b6b,color:#fff
    style T1 fill:#e74c3c,color:#fff
    style T2 fill:#e67e22,color:#fff
    style T3 fill:#27ae60,color:#fff
    style T4 fill:#e67e22,color:#fff
    style T5 fill:#e67e22,color:#fff
    style T6 fill:#e67e22,color:#fff
```

> 6개 테마는 MECE: 크롤링(봇 접근) / 인덱싱(색인 여부) / 프레임워크(구조적 한계) / CWV(사용자 성능) / 콘텐츠(검색 신호) / GEO(AI 검색) — 겹침 없이 기술 SEO 전체를 포괄.

### Branch Registry (일부 발췌)

| # | 가지 | 판정 | 근거 |
|---|------|------|------|
| 1 | 중복 canonical | 🔍 **이슈** | `router.asPath`가 query string 포함 → PageRank 분산 |
| 2 | JS 캐시 헤더 + TinyMCE | 🔍 **이슈** | JS 크롤링 20.86%, TinyMCE 741KB 전역 로드 (실사용 1곳) |
| 3 | 404 비율 | ✅ 기각 | 사이트맵에 삭제 URL 없음, 이전 이슈 잔해 → 모니터링만 |
| 4 | NOINDEX ↔ robots.txt | 🔍 **이슈** | 5개 경로가 noindex만 있고 robots.txt 미차단 → 이중 낭비 |
| 5 | 히어로 배너 SSR 미포함 | 🔍 **이슈** | LCP 요소가 클라이언트 렌더링 후 표시 |
| 6 | 렌더 블로킹 리소스 | 🔍 **이슈** | font.css 동기 로드 + Kakao SDK async 미지정 |
| ... | ... | ... | ... |

### Priority Matrix → 실행 순서

| 우선순위 | 테마 | 공수 | 근거 |
|----------|------|------|------|
| 🔴 긴급 | 크롤링 낭비 제거 | 2.5일 | 공수 대비 효과 최대. 1줄 수정으로 전체 적용 |
| 🟠 높음 | Core Web Vitals | 5.5일 | 현재 양호하나 악화 방지 + 크롤링과 병행 시 시너지 |
| 🟠 높음 | 성능 및 인덱싱 | 2일 | 미색인 36건 해소 |
| 🟠 높음 | 콘텐츠 시그널 | 2일 | 챌린지 페이지 검색 미노출 해소 |
| 🟠 높음 | GEO | 0.5일 | 공수 극소. AI 검색 대응 |
| 🟢 후순위 | Astro 전환 | 장기 | SEO 차이 적으나 인프라 이점 |

### So What?

> **크롤링 예산의 24%가 불필요한 곳에 낭비되고 있다.** canonical 1줄 수정 + TinyMCE lazy load(1.5일)만으로 JS 크롤링 비율을 20% → 10% 이하로 줄일 수 있다. 긴급 항목 2.5일 투자로 가장 큰 효과.

---

## 발표용 예시 모음

발표 시 MECE Diagnostic의 효과를 보여줄 수 있는 개발 시나리오 3가지입니다.

### 예시 1: "배포 후 페이지가 느려졌어요" (외부 팀 확인 요청)

가장 흔한 확인 요청. MECE 없이 하면 "프론트 문제 아닌 것 같아요"로 끝나기 쉬움.

```
Issue Tree (수학적 분해)

  응답 시간 = 네트워크 + 서버 + 클라이언트
               │          │         │
               ▼          ▼         ▼
           CDN 확인    API 확인   번들 확인
           캐시 히트율  응답 시간   사이즈 변화
           98% → ✅    1800ms     변화 없음
           기각         → 🔍       → ✅ 기각

  결과: "5가지 가능성을 모두 확인했습니다.
        API 응답이 200ms → 1800ms로 악화되었고,
        원인은 N+1 쿼리입니다. 백엔드 이슈입니다."

  → 근거가 있으니 상대 팀도 즉시 수긍
```

### 예시 2: "간헐적으로 테스트가 실패합니다" (디버깅)

재현이 어려운 문제일수록 MECE가 빛남. 추측으로 접근하면 끝이 없음.

```
Issue Tree (프로세스 분해 — 테스트 실행 흐름)

  테스트 실패
  ├── 환경 문제
  │   ├── CI 환경과 로컬 차이 → ✅ 기각 (동일 Node 버전 확인)
  │   └── 환경 변수 누락      → ✅ 기각 (.env 비교 완료)
  ├── 테스트 코드 문제
  │   ├── 비동기 타이밍 이슈   → 🔍 이슈! (setTimeout 의존)
  │   ├── 테스트 간 상태 오염  → ✅ 기각 (격리 확인)
  │   └── Mock 데이터 불일치   → ✅ 기각 (스키마 일치)
  └── 외부 의존성
      ├── API 응답 지연        → ✅ 기각 (mock 사용 중)
      └── DB 커넥션 풀         → ✅ 기각 (인메모리 DB)

  → 7개 가지 전수 검증, 1개 이슈 발견.
    "setTimeout(100)에 의존하는 테스트가 CI에서 간헐 실패.
     waitFor로 교체하면 해결됩니다."
```

### 예시 3: "이 기능을 React로 할까 서버로 옮길까?" (아키텍처 의사결정)

정답이 없는 결정일수록 구조화된 비교가 필요.

```
Issue Tree (세그먼트 분해 — 평가 기준별)

  PDF 생성 기능 위치 결정
  ├── 성능
  │   ├── 클라이언트: 브라우저 메모리 제약 → 대용량 PDF 불가
  │   └── 서버: 메모리 유연, 하지만 동시 요청 시 서버 부하
  ├── 사용자 경험
  │   ├── 클라이언트: 즉시 피드백 가능, 오프라인 지원
  │   └── 서버: 생성 대기 시간, 하지만 결과 일관성 보장
  ├── 유지보수
  │   ├── 클라이언트: 브라우저 호환성 관리 비용
  │   └── 서버: 단일 환경, 라이브러리 업데이트 용이
  └── 비용
      ├── 클라이언트: 서버 비용 $0, 하지만 번들 사이즈 증가
      └── 서버: Lambda 비용 발생, 하지만 CDN 캐싱 가능

  Priority Matrix:
  | 기준   | 클라이언트 | 서버 |
  | 성능   | 낮음      | 높음 |
  | UX     | 높음      | 중간 |
  | 유지보수 | 낮음    | 높음 |
  | 비용   | 중간      | 중간 |

  → "4가지 기준으로 비교한 결과, 서버 측 생성을 권장합니다.
     근거: 대용량 PDF 지원 + 유지보수 우위. UX 열위는
     프로그레스바로 보완 가능합니다."
```

> 이 3가지 예시의 공통점: **"확인했습니다" 가 아니라 "N가지를 모두 확인했고, 근거는 이것입니다"**로 답할 수 있다는 것.

---

## Adaptive Depth — 복잡도별 분석 깊이

모든 문제에 동일한 깊이를 적용하지 않습니다. 복잡도를 먼저 판단하고 적절한 레벨을 선택합니다.

| 레벨 | 조건 | 프로세스 | 예시 |
|------|------|----------|------|
| **Light** | 원인 1~2개 추정 | Issue Tree 2단계 → 가설 1개 → 검증 | API 404 반환, 빌드 에러 |
| **Standard** | 원인 복수, 상호작용 가능 | Full Why Tree → 가설 2~3개 → Priority Matrix | 간헐적 테스트 실패, 배포 후 성능 저하 |
| **Deep** | 시스템 레벨, 다수 이해관계자 | 전체 플로우 + 반복 수렴 + What Tree | 아키텍처 전환 결정, 시스템 장애 RCA |

**격상은 있되, 격하는 없다.** 분석 중 복잡도가 예상보다 높으면 즉시 상위 레벨로 올립니다.

---

## 장점

### 1. 빠짐없는 검증 (No Blind Spots)

Branch Registry + Coverage Gate로 **"이 가지는 괜찮겠지"라는 생략을 원천 차단**합니다. 모든 가지를 판정해야만 결론을 내릴 수 있습니다.

### 2. 가설 우선 사고 (Hypothesis-First)

데이터를 먼저 보고 원인을 찾는 귀납적 접근 대신, **가설을 먼저 세우고 반증 데이터를 찾는 연역적 접근**을 강제합니다. 확증 편향을 방지합니다.

### 3. 추적 가능한 분석 (Traceable Reasoning)

Pruning 기록으로 기각된 가설과 그 이유가 모두 남습니다. 동료에게 분석 결과를 공유할 때 **근거를 보여줄 수 있습니다.**

### 4. 적응적 깊이 (Adaptive Depth)

간단한 버그(Light)와 시스템 장애 분석(Deep)에 같은 프로세스를 쓰지 않습니다. 복잡도에 맞는 깊이만 적용하여 **과잉 분석을 방지**합니다.

### 5. 피라미드 원칙 출력

결론이 먼저 오고 근거가 아래에 배치됩니다. 바쁜 동료나 리더가 **첫 문단만 읽어도 핵심을 파악**할 수 있습니다.

---

## 사용법

### 설치

이 레포의 `skill/` 디렉토리를 Claude Code 스킬로 등록합니다.

```bash
cp -r skill/* ~/.claude/skills/mece-diagnostic/
```

### 자동 트리거

다음 상황에서 자동으로 작동합니다:

- 버그/에러 원인 분석
- 성능 저하 조사
- 아키텍처/설계 의사결정
- 코드베이스 탐색 및 이해
- 트레이드오프 평가

### 수동 호출

```
/mece-diagnostic
```

### 스킵 조건

단순 작업에는 적용되지 않습니다:
- rename, format, 단일 파일 수정 등 단순 실행 작업
- 명확한 지시가 있는 구현 작업
- 이미 원인이 확정된 수정 작업

---

## 파일 구조

```
skill/
├── SKILL.md              # 메인 스킬 정의 (프레임워크 전체)
├── complexity-guide.md    # 복잡도 판단 가이드 (Light/Standard/Deep)
└── templates.md           # Issue Tree, Why Tree, 가설 카드 등 템플릿
```

---

## 배경

이 프레임워크는 McKinsey의 문제 해결 방법론을 기반으로 합니다:

- **MECE 원칙** — Mutually Exclusive, Collectively Exhaustive (상호배타적, 전체포괄적)
- **Diagnostic Pruning** — 가설을 세우고 반증으로 가지치기
- **피라미드 원칙** — 결론 먼저, 근거는 아래로
- **Day 1 Answer** — 불완전해도 초기 가설부터 세우기

이를 소프트웨어 엔지니어링 컨텍스트에 맞게 재해석하여, AI 코드 어시스턴트가 구조화된 사고를 하도록 강제하는 스킬로 구현했습니다.

---

## License

MIT
